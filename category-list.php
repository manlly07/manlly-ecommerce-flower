...
<body>
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title d-flex align-items-center justify-content-between">
                Lọc
                <button class="btn btn-primary" data-bs-toggle="offcanvas" data-bs-target="#offcanvasEcommerceCategoryList" aria-controls="DataTables_Table_0">Thêm loại sản phẩm</button>
              </h5>
  ...
  <script>
    ...
    $('#eCommerceCategoryListFormEdit').on('submit', function(e) {
      e.preventDefault();
      var formData = new FormData();
      formData.append('id', $('#id').val());
      formData.append('category_name', $('#ecommerce-category-name-edit').val());
      formData.append('description', $('#ecommerce-category-slug-edit').val());
    ...
    const handleView = (id) => {
      $.ajax({
        ...
        success: (response) => {
          ...
          $('#ecommerce-category-name-edit').val(data.title);
          $('#ecommerce-category-slug-edit').val(data.description);
          $('#id').val(data.id);
